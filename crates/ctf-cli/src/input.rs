...
// ─── Ctrl+O handler ───────────────────────────────────────────────────────────
// Sets a flag then accepts the current line so read_line() can return ShowHistory.
    ...
    pub fn read_line(&mut self) -> io::Result<ReadOutcome> {
        ...
        match self.editor.readline(&self.prompt) {
            Ok(_line) if self.ctrl_o_triggered.swap(false, Ordering::Relaxed) => {
                // Ctrl+O was pressed — discard any buffered text and show history
                writeln!(io::stdout())?;
                Ok(ReadOutcome::ShowHistory)
            }
